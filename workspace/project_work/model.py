...
class Mamba(nn.Module):
    def __init__(self, args: ModelArgs):
        ...
        super().__init__()
        self.args = args
        print(args.vocab_size, args.d_model)
...
class MambaBlock(nn.Module):
    ...
    def selective_scan(self, u, delta, A, B, C, D):
        ...
        delta_u = delta * u
        delta_u_expanded = delta_u.unsqueeze(-1)
        B_expanded = B.unsqueeze(2)
        deltaB_u = delta_u_expanded * B_expanded 
        ...
        x = u.new_zeros((b, d_in, n), device=deltaA.device)
        ys = []    
        for i in range(1):
            x = deltaA[:, i] * x + deltaB_u[:, i]
            vector_C = C[:, i, :].unsqueeze(-1)
            y = torch.matmul(x, vector_C)
            y = y.squeeze(-1)
