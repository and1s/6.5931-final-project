...
@generate_description.register(nn.Conv1d)
def _(module, input, output, name, ifmap_name):
    description = ConvLayerDescription(
        name=name,
        g=module.groups,
        m=output.shape[1],  # Number of output channels
        w=input.shape[2],   # Width of the input
        h=1,                # Height is 1 for 1D convolution
        c=input.shape[1],   # Number of input channels
        n=input.shape[0],   # Batch size
        s=module.kernel_size[0],  # Kernel width
        r=1,                # Kernel height is 1 for 1D convolution
        w_pad=module.padding[0],  # Width padding
        h_pad=0,            # Height padding is 0 for 1D convolution
        w_stride=module.stride[0],  # Width stride
        h_stride=1,         # Height stride is 1 for 1D convolution
        ifmap_name=ifmap_name,
        filter_name=f'{name}_filter',
        ofmap_name=f'{name}_out'
    )
...
def generate_matmul_func(input1, input2, output,
    ...
    if len(input1.shape) == 2 and len(input2.shape) == 2:
        ...
        description = MatrixMatrixMultiplyLayerDescription(
            name = name,
            m = input1.shape[0],
            n = input2.shape[1],
            k = input1.shape[1],
